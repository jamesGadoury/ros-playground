...
Result<ProblemNode<ProblemInterface>> best_first_search(
    ...
    while(!frontier.empty()) {
        ...
        if(problem.goal_state() == node->state) {
            ...
            return {ProblemStatus::Solved, node, expanded_count};
        }

        expanded_count += 1;
        for (std::shared_ptr<Node> child : expand(problem, node)) {
            if (event_dispatcher) event_dispatcher->dispatch(Event::EXPAND, *child);
            if(reached.find(child->state) == reached.end() || child->path_cost < reached[child->state]->path_cost) {
    ...
    return {ProblemStatus::Unsolved, nullptr, expanded_count};
}
